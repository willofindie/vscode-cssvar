...
"use strict";
import type { Input, ProcessOptions } from "postcss";
import type { Token, Tokenizer } from "postcss/lib/tokenize";
...
type ScssTokenizeOptions = ProcessOptions & {
  ignoreErrors?: boolean;
  ignoreUnclosed?: boolean;
};

// SCSS PATCH function name was changed
export default function scssTokenize(
  input: Input,
  options: ScssTokenizeOptions = {}
): Tokenizer {
  const css = input.css.valueOf();
  const ignore = options.ignoreErrors;

  let code, next: number, quote, content, escape;
  let escaped, prev, n, currentToken: Token;
  ...
  function nextToken(opts: ScssTokenizeOptions) {
